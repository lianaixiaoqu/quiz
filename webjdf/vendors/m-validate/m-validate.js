...
;(function(win, undefined){
    ...
    var Mvalidate = function(selector){
    ...
    function alertMessage(opts, message){
        ...
        errorEle.className = 'errorMessageModal';
        var nodeEles = document.getElementsByClassName('errorMessageModal');

        if(nodeEles.length === 0){
            document.getElementsByTagName('body')[0].appendChild(errorEle);
        }
        var errEl = document.getElementsByClassName('errorMessageModal')[0];
        errEl.innerHTML = message;
        errorMessageModalStyle(errEl);
        if(opts.callback){
            opts.callback(this.form[opts.name], errEl)
        }
    }

    function errorMessageModalStyle(errEl){
